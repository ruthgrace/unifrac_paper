...
Unweighted UniFrac is calculated based on the presence or absence of counts for each branch in the phylogenetic tree, when comparing two samples. A branch is unshared when one sample has a non-zero abundance but not the other, and a branch is shared when both samples have a non-zero abundance. The formula for unweighted UniFrac is as follows, where $b$ is the set of branch lengths in the phylogenetic tree, and $A$ and $B$ represent the two samples being compared:

\begin{align*}
Unweighted_{AB} = \frac{\sum_{}{} (b_{A} - b_{B}) \cup (b_{B} - b_{A})}{\sum_{}{} b_{A} \cup b_{B}}
\end{align*}
