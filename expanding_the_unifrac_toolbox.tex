...
Information UniFrac is known to not perform very well for monocultures, due to taxa with very high and low proportional abundances having uncertainty information values close to zero (Fig.~\ref{fig5}).
...
We do not believe that any of these weightings are a perfect model for microbiome data. Each tool is prone to its own set of weaknesses. Information UniFrac and ratio UniFrac are useful for examining data sets that contain the similar sets of taxa between groups. Weighted and ratio UniFrac are useful for examining data sets that are close to monocultures. In any case, inspection should be done to make sure that the tool used accurately represents the data.
